...
def schedule():
    ...
    return jsonify({
        ...
        'sequences': {algo: results[algo]['sequence'] for algo in results}  # Return sequences for all selected algorithms
    })

# Corrected condition to check if the script is run directly
if __name__ == '__main__':  # Corrected from _name_ to __name__ and _main_ to __main__
    app.run(debug=True)
